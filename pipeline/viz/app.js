...
function applyCuratorHash() {
  const was = localStorage.getItem(CURATOR_KEY);
  ...
  return was !== localStorage.getItem(CURATOR_KEY);
}
applyCuratorHash();
// editing the address bar's #hash does NOT reload the page, so apply the switch
// the moment it changes and reload to refresh any open panels
addEventListener("hashchange", () => { if (applyCuratorHash()) location.reload(); });
...
if (curator()) {
  setTimeout(() => toast("Curator mode: source links are visible to you only. "
    + "Add #public to the address to preview the public view."), 1200);
}
