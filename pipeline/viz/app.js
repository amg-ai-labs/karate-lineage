...
/* Output is the curator's, and that now means ALL output, not only images.
   The published site is for reading and for linking to: no charts, no tables,
   no data files. The one thing a reader may still take away is the corrections
   they have typed themselves, which are theirs and are the whole point of
   letting an expert edit in the browser. */
function canExportImages() { return curator(); }
function canExportData() { return curator(); }
// Editing is a curator capability too. The published page is for reading:
// search it, follow a lineage, open a style or a kata, read the analytics. It
// cannot change the record, and it cannot take anything away.
function canEdit() { return curator(); }
function exportFormats(pairs) {
  return canExportData() ? pairs : [];
}
...
function isolateLayout(ids, compact) {
  ...
  // A printed figure inherited the decade columns, and a busy decade holds
  // hundreds of people, so the figure came out as a ribbon: Chojun Miyagi's 761
  // descendants measured 4997 by 9627 and were LESS dense than the map they came
  // from. Splitting an over-long column into side-by-side lanes turns that
  // height into width and gives a shape that fits a page.
  if (compact) {
    const CAP = Math.max(18, Math.min(48, Math.round(Math.sqrt(ids.length * 2.2))));
    const split = [];
    for (const l of layers) {
      if (l.length <= CAP) { split.push(l); continue; }
      const lanes = Math.ceil(l.length / CAP);
      const per = Math.ceil(l.length / lanes);
      for (let i = 0; i < l.length; i += per) split.push(l.slice(i, i + per));
    }
    layers.length = 0;
    for (const l of split) layers.push(l);
  }
  const pIdx = new Map();
...
/* The kata filter, shared by the list and by the export of that list, so the
   document you download is exactly the rows you are looking at. */
function kataNamed(k) {
  return !!(k.origin_person || k.modifier || (k.introduced_by || []).length);
}
function kataMatches(k) {
  const raw = kataQuery.trim(), q = raw.toLowerCase();
  return (!q || k.name.toLowerCase().includes(q)
    || (k.native || "").includes(raw)
    ...
    && (!kataPeopleOnly || kataNamed(k));
}
function visibleKata() {
  const shown = KATA.filter(kataMatches);
  return kataOrder === "alpha"
    ? shown.slice().sort((a, b) => a.name.localeCompare(b.name))
    : shown;
}
...
function renderKataPanel(back) {
  ...
  // The tab itself is a document, not only a route to each kata: the whole list
  // as it is filtered and ordered, at print resolution.
  kataPanel.appendChild(tableExportRow(() => {
    const shown = visibleKata();
    return {
      title: "Kata and forms of karate, kobudō and taekwondo",
      subtitle: shown.length + " of " + KATA.length + " kata"
        + (kataQuery.trim() ? ", matching “" + kataQuery.trim() + "”" : "")
        + (kataDisputedOnly ? ", disputed attributions only" : "")
        + (kataPeopleOnly ? ", with a named person" : ""),
      headers: ["Kata", "Native", "Level", "Origin", "Era", "Likely creator",
                "Modifier", "Styles", "Disputed"],
      rows: shown.map(k => [
        k.name, k.native || "", k.level || "",
        (Object.fromEntries(FAM_ORDER))[k.family] || k.family || "",
        k.era || "", k.origin_person || "", k.modifier || "",
        (k.style_ids || []).map(id => (styleByIdMap.get(id) || {}).label || id).join(", "),
        k.disputed ? "disputed" : "",
      ]),
      fname: "karate-kata" + (kataQuery.trim() ? "-" + slugify(kataQuery.trim()) : ""),
    };
  }));
  ...
  const famLabel = Object.fromEntries(FAM_ORDER);
    ...
    if (kataOpen.has(k.name)) {
      ...
      if (canEdit()) d.appendChild(kataRelationEditor(k));
  ...
  function renderList() {
    ...
    const named = kataNamed;
    const match = kataMatches;
...
/* A chart whose lines carry four distinct meanings cannot rely on the reader
   finding a button. The key opens by itself the first time this browser sees
   the page, and then stays out of the way; it is never drawn on an exported
   figure, where it would compete with the lineage on the page. */
(function () {
  const SEEN = "karate-key-seen";
  let seen = null;
  try { seen = localStorage.getItem(SEEN); } catch (_) { seen = "1"; }
  if (seen) return;
  setTimeout(() => {
    renderKey();
...
function renderKey() {
  ...
  row(line({ stroke: "#d4483f", dash: "4 3" }), "Red.",
      "A link you have marked as wrong in this browser. It is a proposal, not a verdict: "
      + "the line stays drawn until the correction is exported and applied to the data, at "
      + "which point it disappears altogether. Nothing is published in red. A link the "
      + "project itself has judged wrong is not drawn faintly, it is removed.");
...
function renderStatsPanel() {
  ...
  // How far down the ranking to go. A top-ten is a headline; a book chapter
  // wants two hundred, and the export should carry whatever is on screen.
  const many = document.createElement("select"); many.className = "rk-select";
  many.setAttribute("aria-label", "How many people to rank");
  for (const [v, lbl] of [[10, "The first 10"], [25, "The first 25"], [50, "The first 50"],
                          [100, "The first 100"], [250, "The first 250"],
                          [Infinity, "Everyone ranked"]]) {
    const o = document.createElement("option");
    o.value = String(v); o.textContent = lbl; many.appendChild(o);
  }
  many.value = String(rankLimit);
  many.onchange = () => {
    rankLimit = many.value === "Infinity" ? Infinity : parseInt(many.value, 10);
    draw();
  };
  statsPanel.appendChild(many);
  ...
  function draw() {
    ...
    // Every ranking is rebuilt here from each person's own stored figures rather
    // than read from the precomputed top-50, because that list could not answer
    // "show me the first 200" and, when a style was selected, left it with only
    // the handful of its members who had made the global fifty.
    const pool = rankScope ? new Set(styleMembers(rankScope)) : null;
    const rows = [];
    for (const n of DATA.nodes) {
      if (pool && !pool.has(n.id)) continue;
      const v = rankFor(n.id);
      if (!v) continue;
      rows.push({ id: n.id, name: eff(n).name,
                  dates: n.birth ? n.birth + (n.death ? "–" + n.death : "–") : "?",
                  style: n.styleLabel || "", family: n.family || "", v });
    }
    if (terminal) {
      current = rankScope
        ? rows.filter(r => r.v.direct === 0 && r.v.teachers > 0)
    ...
    rankTotal = current.length;
    if (rankLimit !== Infinity) current = current.slice(0, rankLimit);
    if (rankScope) {
      const st = styleByIdMap.get(rankScope);
      note.textContent += " Restricted to " + (st ? st.label : rankScope)
        + " and its sub-styles: " + pool.size + " people, ranked among themselves.";
    }
  ...
  function rankingTable() {
    const measure = RANK.measures.find(x => x.key === rankMeasure);
    const cols = ["Rank", "Person", "Dates", "Style"].concat(currentCols.map(c => c.replace(/_/g, " ")));
    const label = (rankMeasure === "__terminal" ? "terminal-lineages" : rankMeasure)
      + (rankScope ? "-in-" + slugify(rankScope) : "");
    return {
      title: rankMeasure === "__terminal"
        ? "Lineages that ended with their holder"
        : (measure ? measure.title : "Ranking"),
      subtitle: current.length + " of " + rankTotal + " ranked"
        + (rankScope ? ", within " + ((styleByIdMap.get(rankScope) || {}).label || rankScope)
                       + " and its sub-styles" : "")
        + (measure && measure.note ? ". " + measure.note : ""),
      headers: cols,
      rows: current.map((r, i) => [rankMeasure === "__terminal" ? "" : String(i + 1),
                                   r.name, r.dates, r.style]
        .concat(currentCols.map(c => String(r.v[c] === undefined ? "" : r.v[c])))),
      fname: "karate-analytics-" + label,
    };
  }
  function downloadTable() { exportTable(rankingTable(), "csv"); }
  statsPanel.appendChild(tableExportRow(rankingTable));
  draw();
}
...
function renderStylePanel() {
  ...
  stylePanel.appendChild(tableExportRow(() => {
    const fam = Object.fromEntries(FAM_ORDER);
    const rows = [];
    for (const [fid] of FAM_ORDER)
      for (const [s, depth] of familyStyleTree(fid))
        rows.push([s.label, fam[fid] || fid, String(depth + 1),
                   styleChainLabel(s.id), s.founder || "", s.founded || "",
                   String(styleMembers(s.id).length)]);
    return {
      title: "Styles of karate, kobudō and taekwondo",
      subtitle: rows.length + " styles, sub-styles and sub-sub-styles, by originating tradition",
      headers: ["Style", "Originating group", "Level", "Ancestry", "Founder", "Founded", "People"],
      rows: rows,
      fname: "karate-styles",
    };
  }));
...
function openDetail(id, back) {
  ...
  for (const [title, es, pick] of rels) {
    ...
    if (!canEdit()) return;          // proposing a new link is an edit
  ...
  /* --- expert editing: names and dates. Curator only, like every output. --- */
  if (!canEdit()) return;
...
function renderListPanel(p, people, blurb) {
  ...
  p.appendChild(tableExportRow(() => ({
    title: blurb,
    subtitle: people.length + (people.length === 1 ? " person" : " people"),
    headers: ["Person", "Native", "Born", "Died", "Style", "Students", "Descendants"],
    rows: people.map(n => { const f = eff(n); return [
      f.name, f.native || "", f.birth || "", f.death || "",
      n.styleLabel || "", String(n.students || 0), String(n.desc || 0)]; }),
    fname: "karate-" + slugify(blurb),
  })));
...
expBtn.onclick = ev => {
  ev.stopPropagation();
  // A published build offers nothing: no figures, no data, no corrections,
  // because it cannot make corrections either.
  for (const b of expMenu.querySelectorAll("button"))
    b.hidden = !canExportData();
...
function buildExportSVG(opts) {
  ...
  // No key on the figure. A book plate wants the lineage and nothing competing
  // with it; a row of swatches and four dash patterns along the foot is clutter
  // at the size these are printed. The key lives in the application, where it
  // can be read at leisure, and a caption in the book can carry whatever of it
  // the figure actually needs.
...
function exportSVG(opts) {
  const { out, W, H } = (opts && opts.svgDoc) ? opts.svgDoc : buildExportSVG(opts);
...
function rasterise(opts, scale, maxArea, cb) {
  // opts.svgDoc lets anything that can draw itself as SVG use the whole image
  // pipeline: PNG, JPG, 300 dpi TIFF and print PDF. Without it the kata tab and
  // the style list had no image export at all, because the only SVG the
  // application could build was a cladogram.
  const { out, W, H } = (opts && opts.svgDoc) ? opts.svgDoc : buildExportSVG(opts);
...
/* ============ tables as documents: the kata tab, the style list ============

   A list is not a cladogram, so it could not use buildExportSVG, and that is
   the whole reason the kata tab offered no PDF, TIFF or JPEG. Drawing the list
   as an SVG page puts it back on the same pipeline as every other export, at
   the same 300 dpi, with no dead space: columns are sized to their contents and
   the page is exactly as tall as the rows need.                              */
const TBL = { row: 26, head: 34, pad: 34, title: 78, cell: 12, chw: 6.6, gap: 22 };

function buildTableSVG(title, subtitle, headers, rows) {
  const widths = headers.map((h, i) => {
    let w = String(h).length * TBL.chw * 1.05;
    for (const r of rows) w = Math.max(w, String(r[i] == null ? "" : r[i]).length * TBL.chw);
    return Math.min(Math.max(w, 60), 460) + TBL.gap;
  });
  const W = widths.reduce((a, b) => a + b, 0) + TBL.pad * 2;
  const H = TBL.title + TBL.head + rows.length * TBL.row + TBL.pad * 2;
  const ink = cssVar("--ink") || "#0b0b0b", muted = cssVar("--muted") || "#898781";
  const grid = cssVar("--grid") || "#e1e0d9", surface = cssVar("--surface") || "#fcfcfb";
  const out = el("svg", { xmlns: SVGNS, viewBox: `0 0 ${W} ${H}`, width: W, height: H,
                          "font-family": 'system-ui, -apple-system, "Segoe UI", sans-serif' });
  el("rect", { x: 0, y: 0, width: W, height: H, fill: surface }, out);
  const text = (s, x, y, size, fill, weight) => {
    const t = el("text", { x: x, y: y, "font-size": size, fill: fill }, out);
    if (weight) t.setAttribute("font-weight", weight);
    t.textContent = s == null ? "" : String(s);
    return t;
  };
  text(title, TBL.pad, TBL.pad + 26, 22, ink, 700);
  if (subtitle) text(subtitle, TBL.pad, TBL.pad + 48, 12.5, muted);

  let y = TBL.pad + TBL.title;
  let x = TBL.pad;
  headers.forEach((h, i) => {
    text(String(h).toUpperCase(), x, y + 16, 10, muted, 600);
    x += widths[i];
  });
  el("line", { x1: TBL.pad, y1: y + 24, x2: W - TBL.pad, y2: y + 24,
               stroke: cssVar("--axis") || "#c3c2b7", "stroke-width": 1 }, out);
  y += TBL.head;
  rows.forEach((r, ri) => {
    if (ri % 2) el("rect", { x: TBL.pad - 6, y: y - 16, width: W - TBL.pad * 2 + 12,
                             height: TBL.row, fill: cssVar("--band") || "rgba(11,11,11,.022)" }, out);
    let cx = TBL.pad;
    r.forEach((cell, i) => {
      const room = Math.floor((widths[i] - TBL.gap) / TBL.chw);
      let s = cell == null ? "" : String(cell);
      if (s.length > room) s = s.slice(0, Math.max(1, room - 1)) + "…";
      text(s, cx, y, TBL.cell, i === 0 ? ink : (cssVar("--ink-2") || "#52514e"), i === 0 ? 600 : null);
      cx += widths[i];
    });
    el("line", { x1: TBL.pad, y1: y + 9, x2: W - TBL.pad, y2: y + 9,
                 stroke: grid, "stroke-width": 0.6 }, out);
    y += TBL.row;
  });
  text(DATA.meta.title || "The Lineage of Karate and Taekwondo", TBL.pad, H - TBL.pad + 6, 10, muted);
  return { out: out, W: W, H: H };
}

// A standalone HTML page of the same table: opens in a browser, pastes into
// Word or InDesign with the table intact, and needs nothing to view it.
function tableHTML(title, subtitle, headers, rows) {
  ...
  return "<!doctype html>\n<html lang=\"en\"><head><meta charset=\"utf-8\">\n<title>"
    ...
    + "background:#fcfcfb;margin:32px;max-width:100%}\n"
    + "h1{font-size:20px;margin:0 0 4px}p.sub{color:#52514e;margin:0 0 18px;font-size:12.5px}\n"
    + "table{border-collapse:collapse;width:100%}th{text-align:left;font-size:10px;"
    + "text-transform:uppercase;letter-spacing:.05em;color:#898781;border-bottom:1px solid #c3c2b7;"
    + "padding:6px 10px 6px 0}\n"
    + "td{padding:5px 10px 5px 0;border-bottom:1px solid #e1e0d9;vertical-align:top}\n"
    + "tr:nth-child(even) td{background:rgba(11,11,11,.022)}\n"
    + "td:first-child{font-weight:600}\n"
    + "@media print{body{margin:12mm}tr{break-inside:avoid}}\n"
    + "@media(prefers-color-scheme:dark){body{background:#0d0d0d;color:#fff}"
    + "td{border-bottom-color:#2c2c2a}p.sub,th{color:#c3c2b7}}\n"
    + "</style></head><body>\n<h1>" + esc(title) + "</h1>\n"
...
// One dispatcher for every list export, so a tab cannot end up offering data
// formats but no images again.
function exportTable(cfg, fmt) {
  const { title, subtitle, headers, rows, fname } = cfg;
  if (!rows.length) { toast("Nothing to export: no rows match the current filter."); return; }
  if (fmt === "csv") return saveCSV([headers].concat(rows), fname + ".csv");
  if (fmt === "json")
    return download(fname + ".json", new Blob(
      [JSON.stringify(rows.map(r => Object.fromEntries(headers.map((h, i) => [h, r[i]]))), null, 1)],
      { type: "application/json" }));
  if (fmt === "html")
    return download(fname + ".html", new Blob([tableHTML(title, subtitle, headers, rows)],
      { type: "text/html" }));
  const opts = { svgDoc: buildTableSVG(title, subtitle, headers, rows), fname: fname };
  if (fmt === "svg") return exportSVG(opts);
  if (fmt === "pdf") return exportPDF(opts);
  if (fmt === "tiff") return exportTIFF(opts);
  if (fmt === "jpg") return exportJPG(4, opts);
  return exportPNG(4, opts);
}

// The buttons a list offers. Images are the curator's, as everywhere else.
function tableExportRow(cfg) {
  const wrap = document.createElement("div");
  wrap.style.cssText = "display:flex;gap:6px;flex-wrap:wrap;margin:8px 0 4px";
  for (const [lbl, fmt] of exportFormats(
        [["⬇ SVG", "svg"], ["PDF", "pdf"], ["TIFF", "tiff"], ["JPG", "jpg"], ["PNG", "png"],
         ["HTML", "html"], ["CSV", "csv"], ["JSON", "json"]])) {
    const b = document.createElement("button");
    b.className = "btn"; b.textContent = lbl;
    b.onclick = () => exportTable(typeof cfg === "function" ? cfg() : cfg, fmt);
    wrap.appendChild(b);
  }
  const n = document.createElement("div"); n.className = "edit-note";
  n.textContent = canExportImages()
    ? "The whole list as it is currently filtered and ordered, at 300 dpi for print."
    : "The whole list as data. Print-resolution images are generated from the curator's copy.";
...
// The toolbar must not offer what the build cannot do: on a published copy the
// export menu is empty and there is nothing to edit or add, so the three
// buttons go rather than sit there disabled.
if (!canEdit()) {
  for (const id of ["editmode", "addperson"]) {
